<x-app-layout>
    ...
    <style>
        ...
        .de-file-upload:hover {
            border-color: var(--de-violet);
            background: #f5f3ff;
        }

        .de-file-upload__icon {
            display: flex;
            align-items: center;
            justify-content: center;
            width: 4rem;
        ...
        .de-file-upload__icon svg {
            width: 2rem;
            height: 2rem;
        }

        .de-file-upload__text {
            font-weight: 600;
            color: var(--de-ink);
            text-align: center;
        }

        .de-file-upload__subtext {
            margin-top: .5rem;
            font-size: .875rem;
            color: var(--de-muted);
            text-align: center;
    ...
    <x-slot name="header">
        ...
        <div class="de-header-row de-font">
            <div>
                ...
                <h1 class="de-title">Edit Car</h1>
                <p class="de-subtitle">Update the details for {{ $car->brand }} {{ $car->model }}.</p>
            </div>
                ...
                <form action="{{ route('cars.update', $car) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="de-form-grid">
                        ...
                        <div class="de-form-group">
                            ...
                            <input type="text" name="brand" value="{{ old('brand', $car->brand) }}" class="de-input"
                                placeholder="e.g. Toyota">
                        </div>

                        <div class="de-form-group">
                            <label class="de-label">Model</label>
                            <input type="text" name="model" value="{{ old('model', $car->model) }}" class="de-input"
                                placeholder="e.g. Camry">
                        </div>

                        <div class="de-form-group">
                            <label class="de-label">Year</label>
                            <input type="number" name="year" value="{{ old('year', $car->year) }}" class="de-input"
                                placeholder="e.g. 2024">
                        </div>

                        <div class="de-form-group">
                            <label class="de-label">Seats</label>
                            <input type="number" name="seats" value="{{ old('seats', $car->seats) }}" class="de-input"
                                placeholder="e.g. 5">
                        </div>
                        ...
                        <div class="de-form-group">
                            ...
                            <input type="number" step="0.01" name="price_per_day"
                                value="{{ old('price_per_day', $car->price_per_day) }}" class="de-input"
                    ...
                    <div class="de-form-group mt-6">
                        ...
                        <textarea name="description" rows="4" class="de-input"
                            placeholder="Provide details about the car condition, history, or special notes...">{{ old('description', $car->description) }}</textarea>
                    </div>
                            ...
                            @foreach ($features as $feature)
                                <label class="de-checkbox-card">
                                    ...
                                    <input type="checkbox" name="features[]" value="{{ $feature->id }}"
                                        class="de-checkbox-input" {{ in_array($feature->id, old('features', $car->features->pluck('id')->toArray())) ? 'checked' : '' }}> <span
                                        class="de-checkbox-label">{{ $feature->name }}</span>
                                </label>
                            @endforeach
                        </div>
                    </div>
                    ...
                    <div class="de-form-group mt-6">
                        ...
                        <label class="de-file-upload">
                            ...
                            <span class="de-file-upload__text">Click to upload a new image or drag and drop</span>
                            <span class="de-file-upload__subtext">Leave empty to keep the current image</span>
